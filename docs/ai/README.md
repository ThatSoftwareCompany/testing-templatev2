...
Authentication is template-managed infrastructure. Agents must preserve its migrations, cookie contract, CSRF enforcement, Argon2id password parameters, Ed25519/EdDSA JWT validation, refresh-token rotation, and deny-by-default permission boundary. Product routes belong in `internal/app/routes.go` and new business modules, not in the auth or operational composition files.
